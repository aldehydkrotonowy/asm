#!/usr/bin/perl -W
#
# konw-gas.pl - Skrypt konwertujacy pliki naglowkowe jezyka C na pliki naglowkowe dla GNU as
# konw-gas.pl - A script which converts C header files to GNU as-compatible header files
#
...
#		./konw-gas.pl [--help] [--license] [--prefix <string>] [--suffix <string>] xxx.h [yyy.inc]
#
...
$help_msg = "$0:\n Skrypt konwertujacy pliki naglowkowe jezyka C na pliki naglowkowe dla GNU as\n".
	"  / A script which converts C header files to GNU as-compatible header files.\nAutor/Author: Bogdan Drozdowski, ".
...
{
	my $symbol = shift;
	my $ret = ".extern\t$prefix$symbol$suffix\n";
	if ( $prefix || $suffix )
	{
		$ret .= ".equ $symbol,$prefix$symbol$suffix\n";
	}
	return $ret;
};
...
my $comment_sub = sub
{
	# GNU as accepts C/C++ comments, so don't substitute anything
...
my $preproc_sub = sub
{
	...
	if ( $line =~ /^\s*#\s*define\s+/io )
	{
		...
		$line =~ s/\s*#\s*define\s+(\w+)\s*$/.equ $1,1\n/i;
		...
		$line =~ s/\s*#\s*define\s+(\w+)\s+(.*)/.equ $1,$2/i;
	...
	if ( $line =~ /^\s*#\s*else\s+if/io )
	{
		...
		$line =~ s/\s*#\s*else\s+if(.*)$/.elseif $1/i;
		return $line;
	}
	...
	if ( /^\s*#\s*warning/io ) {
		...
		$line =~ s/\s*#\s*warning(.*)$/.warning\t$1/i;
		return $line;
	}
...
my $typedef_sub = sub
{
	...
	my $part2 = shift;
	my $ret = '';
	# GNU as doesn't accept spaces in the definition, so skip such typedefs
	if ( $part1 !~ /\s/o ) {
		$ret = ".equ $part2,$part1\n";
	}
	return $ret;
...
{
	# In GNU as, structures from a separate section. Since it's not always
	# possible to go back to the previous section, structures are not
	# supported here.
	return '';
};
...
my $struct_end_sub = sub
{
	...
	# In GNU as, structures from a separate section. Since it's not always
	# possible to go back to the previous section, structures are not
	# supported here. But if a variable name follows the structure definition,
	# an appropriate number of bytes is reserved.
	if ( $name )
	{
		$ret .= "$name:\n";
		$ret .= ".fill $size\n" if $size;
