...
CZYTAJ: while ( <$we> ) {
	...
	if ( /^\s*\.globa?l/io ) 	{ s/^\s*\.globa?l\s*(.*)/global\t$1/i; print $wy "$_"; next; }
	...
	if ( /^\s*\.extern/io ) 	{ s/^\s*\.extern\s*(.*)/extern\t$1/i; print $wy "$_"; next; }
	...
	if ( /^\s*\.rept/io ) 		{ s/^\s*\.rept\s*(.*)/\%rep $1/i; $repeat = 1; print $wy "$_"; next; }
	...
	if ( /^\s*\.endm/io )		{ s/^\s*\.endm.*$/\%endmacro\n/io; print $wy "$_"; next; }
	...
	if ( /^\s*\.section/io )	{ s/^\s*\.section\s+([\w\-\.]+).*$/section "$1"/i; print $wy "$_"; next; }
	...
	if ( /^\s*\.code\d{2}/io ) 	{ s/^\s*\.code(\d{2})/bits $1/i; print $wy "$_"; next; }

	if ( /^\s*\.type/io )		{ print $wy "\n"; next; }

	if ( /^\s*\.size/io )		{ print $wy "\n"; next; }

	if ( /^\s*\.p2align/io )	{ s/^\s*\.p2align\s*(\d+)\s*,.*/align 1 << $1/i; print $wy "$_"; next; }
	...
	# ==================== Sekcje (sections)
	if ( /^\s*\.data/io ) 		{ s/^\s*\.data\s*.*$/section .data\n/io; print $wy "$_"; next; }

	if ( /^\s*\.text/io ) 		{ s/^\s*\.text\s*.*$/section .text\n/io; print $wy "$_"; next; }

	if ( /^\s*\.bss/io ) 		{ s/^\s*\.bss\s*.*$/section .bss\n/io; print $wy "$_"; next; }
	...
	if ( /\s*\.zero/io ) 		{ s/\s*\.zero\s*(\d+)/ resb $1/i; print $wy "$_"; next; }
