#!/usr/bin/perl -W
#
# konw-nasm.pl - Skrypt konwertujacy pliki naglowkowe jezyka C na pliki naglowkowe dla NASMa
# konw-nasm.pl - A script which converts C header files to NASM-compatible header files
#
...
#		./konw-nasm.pl [--help] [--license] [--prefix <string>] [--suffix <string>] xxx.h [yyy.inc]
#
...
my $preproc_sub = sub
{
	...
	if ( $line =~ /^\s*#\s*define\s+/io )
	{
		...
		$line =~ s/\s*#\s*define\s+(\w+)\s*$/\%define $1\t1\n/i;
		...
		$line =~ s/\s*#\s*define\s+(\w+)\s+(.*)/\%define $1\t$2\n/i;
	...
	if ( $line =~ /^\s*#\s*error/io || /^\s*#\s*warning/io )
	{
		...
		$line =~ s/\s*#\s*(error|warning)(.*)$/\%error "$2"\n/i;
		return $line;
	}
